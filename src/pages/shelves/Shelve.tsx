...
const Shelve = observer(({ books, actions, dialogBookClickHandler }: TShelve) => {
      ...
      <Dialog onClose={handleDialogClose} open={isBookDialogOpenned} fullWidth={true}>
          ...
          <Grid container justify="center" spacing={2}>
            <Grid item >
              {selectedBook && (
                <BookItem
                  book={selectedBook}
                  onClick={() => dialogBookClickHandler && dialogBookClickHandler(selectedBook)}
                />
              )}
            </Grid>
            <Grid item xs={8}>
        ...
        <DialogActions>
          {actions.map((action, index) => (
            <Button key={index} onClick={() => handleBookAction(action)} color="primary">
              {action.text}
            </Button>
          ))}
        </DialogActions>
